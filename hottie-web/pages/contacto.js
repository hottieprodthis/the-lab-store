import Head from 'next/head';
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';

const contactLinks = [
  {
    name: 'Email',
    value: 'hottieprodthis@gmail.com',
    href: 'mailto:hottieprodthis@gmail.com',
    label: 'Enviar correo',
    icon: (
      <svg className="w-6 h-6 text-volt" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
      </svg>
    )
  },
  {
    name: 'WhatsApp',
    value: '+34 634 670 480',
    href: 'https://wa.me/34634670480',
    label: 'Escribir por WhatsApp',
    icon: (
      <svg className="w-6 h-6 text-volt" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/>
      </svg>
    )
  },
  {
    name: 'Instagram',
    value: '@hottieprodthis',
    href: 'https://instagram.com/hottieprodthis',
    label: 'Ir a Instagram',
    icon: (
      <svg className="w-6 h-6 text-volt" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
      </svg>
    )
  },
  {
    name: 'Discord',
    value: 'hottieprod',
    href: 'https://discord.com',
    label: 'Usuario: hottieprod',
    icon: (
      <svg className="w-6 h-6 text-volt" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path d="M20.317 4.37a19.791 19.791 0 00-4.885-1.515.074.074 0 00-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 00-5.487 0 12.64 12.64 0 00-.617-1.25.077.077 0 00-.079-.037A19.736 19.736 0 003.677 4.37a.07.07 0 00-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 00.031.057 19.9 19.9 0 005.993 3.03.078.078 0 00.084-.028c.462-.63.874-1.295 1.226-1.994.021-.041.001-.09-.041-.106a13.107 13.107 0 01-1.872-.892.077.077 0 01-.008-.128 10.2 10.2 0 00.372-.292.074.074 0 01.077-.01c3.928 1.793 8.18 1.793 12.061 0a.074.074 0 01.078.01c.12.098.246.198.373.292a.077.077 0 01-.006.127 12.299 12.299 0 01-1.873.892.077.077 0 00-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 00.084.028 19.839 19.839 0 006.002-3.03.077.077 0 00.032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 00-.031-.028zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/>
      </svg>
    )
  }
];

export default function Contacto() {
  return (
    <>
      <Head>
        <title>Contacto — The Lab</title>
      </Head>

      <Navbar />

      <main className="mx-auto max-w-3xl px-5 py-20 text-paper">
        <section className="text-center">
          <h1 className="font-display text-4xl sm:text-5xl uppercase tracking-wide">
            HABLEMOS
          </h1>
          <p className="mt-4 text-muted text-sm sm:text-base">
            ¿Quieres reservar un servicio, consultar por un pack o tienes alguna pregunta? Escríbeme directamente por cualquier canal.
          </p>
        </section>

        <section className="mt-12 grid gap-6 sm:grid-cols-2">
          {contactLinks.map((item, idx) => (
            <div 
              key={idx} 
              className="flex flex-col justify-between rounded-xl border border-white/10 bg-surface/50 p-6 transition hover:border-volt/50"
            >
              <div>
                <div className="flex items-center gap-3 mb-3">
                  {item.icon}
                  <span className="text-xs font-semibold uppercase tracking-widest text-volt">
                    {item.name}
                  </span>
                </div>
                <p className="text-lg font-bold text-paper">
                  {item.value}
                </p>
              </div>
              <a
                href={item.href}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-6 inline-block rounded-sm bg-volt px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-widest text-ink transition hover:brightness-110"
              >
                {item.label}
              </a>
            </div>
          ))}
        </section>

        {/* Sección de Horario */}
        <section className="mt-12 text-center rounded-xl border border-white/10 bg-surface/50 p-8">
          <h2 className="text-xl sm:text-2xl font-bold uppercase tracking-wider text-volt mb-3">
            Horario
          </h2>
          <p className="text-base sm:text-lg font-medium text-paper">
            Lunes a Viernes
          </p>
          <p className="text-lg sm:text-xl font-bold text-volt underline decoration-volt underline-offset-4 mt-1">
            16:00 - 21:00
          </p>
          <p className="text-xs text-muted mt-3">
            Hora de España Peninsular (CET)
          </p>
        </section>
      </main>

      <Footer />
    </>
  );
}
